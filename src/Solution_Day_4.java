...
public class Solution_Day_4 {
    ...
    public Solution_Day_4(){
        ...
        for(String s : guardHabitsToAnalyse){
            try{
                ...
                guard.setGuardEvent(temp.replaceAll("]", "").trim());

                guardEvents.add(guard);
    ...
    public String part1(){
        ...
        for(GuardEvents guard : guardEvents){
            if(guard.getId() != 0){
                ...
                if(guard.getGuardEvent().equals("falls asleep")){
                    ...
                    long diff = guard.getDateTime().getTime() - fallsAsleep.getTime();
                    long diffMinutes = diff / (60 * 1000) % 60;
                    minutesGuardIsSleeping.put(id, minutesGuardIsSleeping.getOrDefault(id, 0L) + diffMinutes);
                }
            }
        }
        ...
        int idOfGuardSleptMost = 0;
        int minutesSlept = 0;

        while(iterator.hasNext()){
            Map.Entry sleeping = (Map.Entry)iterator.next();
            long tempVal = (Long) sleeping.getValue();
            int minutes = (int) tempVal;

            if(minutesSlept < minutes){
                idOfGuardSleptMost = (Integer) sleeping.getKey();
